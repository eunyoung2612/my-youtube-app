...
def download_korean_font():
    """
    워드클라우드에서 한글이 네모(□)로 깨지지 않도록,
    나눔고딕 폰트 파일을 인터넷에서 내려받아 임시 파일로 저장합니다.

    @st.cache_resource를 붙여두면, 앱이 다시 실행되어도
    한 번 내려받은 폰트는 다시 내려받지 않고 재사용합니다.
...
# =========================================================
# 댓글로 워드클라우드 이미지를 만드는 함수
# =========================================================
def make_wordcloud_image(comments_list, font_path: str):
    """
    댓글 리스트를 받아서 워드클라우드 이미지를 만듭니다.
    (2단계와 동일하게 한 글자짜리 단어는 제외합니다.)

    matplotlib을 쓰지 않고, wordcloud 라이브러리가 만들어주는
    PIL 이미지를 그대로 돌려줍니다.
    """
    # 모든 댓글을 하나의 텍스트로 합칩니다.
    full_text = " ".join(c["댓글"] for c in comments_list)

    # 단어를 뽑아낸 뒤, 한 글자짜리 단어는 제외하고 다시 하나의 문자열로 합칩니다.
    # (WordCloud는 텍스트를 통째로 받아서 자체적으로 단어를 나누기 때문에,
    #  한 글자 단어를 미리 걸러낸 "정제된 텍스트"를 만들어서 넘겨줍니다.)
    raw_words = re.findall(r"\w+", full_text, flags=re.UNICODE)
    ...
    # matplotlib 없이 바로 이미지(PIL Image)로 변환합니다.
...
if not video_id:
    ...
    if not api_key:
        ...
        if error:
            ...
            if not font_path:
                st.warning(
                    "⚠️ 워드클라우드에 쓸 한글 폰트를 내려받지 못했어요.\n\n"
                    "인터넷 연결 상태를 확인하시거나, 잠시 후 다시 시도해주세요."
                )
            else:
